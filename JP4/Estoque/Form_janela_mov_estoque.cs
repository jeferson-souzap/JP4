...
namespace JP4
{
    public partial class STOQ01 : Form
    {
        ...
        {
            try
            {   // Atualizado dia >> 28/01/2022 // Foi refeito o filtro dentro da string de conecção do banco de dados
                // Caso tenha algum erro de conulta daqui pra frente verificar a string// 
                // Salva no Git

                string cod_operacao = "APON";
                string cod_operacao1 = "BACK";
                int mes_atual = DateTime.Today.Month;

                string conecta_string = Properties.Settings.Default.db_aplicativo_kpiConnectionString;
                string comando_sql = "select id_estoque_trans, cod_descri_completa, cod_operacao, num_docum, qtd_movto, secao_nome, data_operac, observacao, mes_movto, ano_movto, cod_local_est_orig, cod_local_est_dest from estoque_trans " +
                    "where (cod_operacao <>'" + cod_operacao + "' AND cod_operacao <>'"+ cod_operacao1 + "') AND (qtd_movto >= 0 AND status_estorno = 0 AND mes_movto ="+ mes_atual + ") order by id_estoque_trans desc";
